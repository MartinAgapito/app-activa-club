...
const { createMemberCognitoUser } = await import('./cognito');

describe('createMemberCognitoUser', () => {
  beforeEach(() => {
    send.mockReset();
  ...
  it('crea el usuario con contraseña definitiva y lo agrega al grupo member', async () => {
    ...
    const cognitoSub = await createMemberCognitoUser({
    ...
    expect(createInput.Username).toBe('nuevo@example.com');
  ...
  it('traduce UsernameExistsException a EMAIL_ALREADY_USED (409)', async () => {
    ...
    await expect(
      createMemberCognitoUser({ email: 'repetido@example.com', password: 'Sup3rSecreta!' }),
  ...
  it('propaga otros errores de Cognito sin traducirlos', async () => {
    ...
    await expect(
      createMemberCognitoUser({ email: 'nuevo@example.com', password: 'Sup3rSecreta!' }),
