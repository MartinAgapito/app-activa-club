...
(`dev`, `demo`/`prod`), pipelines de GitHub Actions, aprobaciones manuales para
...
- [`terraform-infraestructura.md`](./terraform-infraestructura.md) — Base de
  ...
  estado (local hoy, plan de migración a S3+DynamoDB), entornos `dev`/`demo`,
...
- Despliegue a desarrollo y despliegue final/demo: pendientes de historias
