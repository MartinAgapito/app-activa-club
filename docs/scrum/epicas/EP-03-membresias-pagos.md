...
> **Cambio de pasarela (2026-08-09)** — Esta épica se planificó y ejecutó contra **Culqi sandbox**. El Product Owner decidió migrar a **Stripe (test mode)** porque Culqi exige RUC para emitir credenciales incluso de sandbox, requisito que este proyecto de tesis no puede cumplir. El **alcance funcional de la épica no cambia**: las mismas capacidades, las mismas reglas de negocio y los mismos endpoints. Ver [ADR-0011](../../architecture/adr/ADR-0011-stripe-sandbox-reemplaza-culqi.md) y [US-037](../historias/US-037-migrar-pasarela-culqi-a-stripe.md).

## Descripción

Entregar el segundo bloque funcional de negocio de Activa Club: convertir una cuenta digital ya existente (creada por activación o por registro + aprobación en EP-02) en un **socio activo al día**, mediante el pago digital de su membresía mensual o anual con tarjeta a través de **Stripe (test mode)**.

La épica cubre la consulta de planes disponibles, el pago de la primera membresía tras la aprobación administrativa, el pago de renovación (incluida la deuda de un socio migrado con membresía vencida), la autorización explícita de renovación automática, la confirmación asíncrona del pago mediante el webhook de Stripe y el historial de pagos para socio y administrador. Todo el flujo respeta la separación cliente/servidor y la idempotencia definidas en [ADR-0011](../../architecture/adr/ADR-0011-stripe-sandbox-reemplaza-culqi.md), que reemplaza a [ADR-0007](../../architecture/adr/ADR-0007-culqi-sandbox-idempotencia-pagos.md) conservando sus garantías.
...
- Infraestructura de endpoints serverless de membresías y pagos provisionada en Terraform, con la llave secreta de Stripe resguardada como secreto gestionado (nunca en el repositorio).
...
- Pago de la **primera membresía** con tarjeta vía Stripe (test mode), que transiciona al socio a `ACTIVE` con `membershipEndsAt` correcto (cierra RN-ACT-07).
...
- **Idempotencia** del cobro: la misma `idempotencyKey` nunca genera dos cargos, con doble protección (ítem en DynamoDB + `Idempotency-Key` nativo de Stripe; ADR-0011 §D4, RT-01).
- **Confirmación segura** del estado de membresía: síncrona por respuesta del PaymentIntent y asíncrona por webhook de Stripe con firma `Stripe-Signature` verificada, convergiendo de forma idempotente (RN-PAG-07).
- **Historial de pagos**: propio para el socio, filtrado para el administrador.
- **Manejo seguro de datos de pago** verificado extremo a extremo: ni PAN, ni CVC, ni secretos de Stripe en requests, respuestas, logs o DynamoDB (RN-PAG-08).
...
| [US-037](../historias/US-037-migrar-pasarela-culqi-a-stripe.md)         | Migrar la pasarela de pagos de Culqi a Stripe (test mode)       | Backend + Frontend + DevOps | US-019..US-026 |

> US-037 es **deuda técnica arrastrada de esta épica**, planificada en el Sprint 3 (ver su sección "Ubicación en el backlog"): sin ella, ningún socio puede llegar a `ACTIVE` pagando y EP-03 queda funcionalmente abierta.
...
- No existe PAN, CVC ni secreto de Stripe en ningún request, respuesta, log de CloudWatch, ítem de DynamoDB ni archivo del repositorio (RN-PAG-08).
...
La matriz de alcance §3 clasifica como MVP la "renovación automática opcional con autorización explícita (opt-in del socio)". Los contratos vigentes cubren la **autorización** (`PATCH /members/me/auto-renew`, `autoRenew` en `POST /payments`) pero **no** la ejecución de un cargo recurrente desatendido (tarjeta en archivo + planificador), que no está contemplada ni en ADR-0007 ni en ADR-0011. EP-03 entrega la autorización explícita, su revocación y su visibilidad; la ejecución automática del cargo requiere una decisión previa del Arquitecto (ADR complementario) sobre cargos recurrentes en Stripe (`setup_future_usage` / `off_session`), hoy fuera de alcance. Ver US-023 y los riesgos del Sprint 2.
