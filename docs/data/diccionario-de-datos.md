...
| `AvailabilitySlotStatus` | `AVAILABLE`, `RESERVED`, `MAINTENANCE`, `PAST`                                                                                                                                                                                                                                       | Motivo del estado de una franja de disponibilidad (calculado, no persistido) | RN-RES-07/11          |
...
| `guestName`           | string            | sí   | Nombre completo del invitado, copiado del `GuestProfile` resuelto (no del texto enviado en la petición) |
...
| Atributo            | Tipo         | Nulo | Descripción                                                      |
| ------------------- | ------------ | ---- | ---------------------------------------------------------------- |
| `guestDni`          | string       | no   | DNI del invitado externo (identidad del perfil)                  |
| `firstName`         | string       | no   | Nombre; se fija en el primer registro y no se pisa después       |
| `lastName`          | string       | no   | Apellido; misma regla                                            |
| `createdByMemberId` | string       | no   | Socio titular que lo registró la primera vez                     |
| `createdAt`         | string (ISO) | no   | Alta del perfil (dentro de la transacción de su primera reserva) |
| `updatedAt`         | string (ISO) | no   | Última reserva en la que participó                               |
