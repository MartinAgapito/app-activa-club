...
RN-PAG-03). Deliberadamente **desactivada por defecto**: ninguna cuenta
queda con `autoRenew=true` sin esta acción explícita. La identidad sale
siempre del JWT — no hay `memberId` en la ruta ni en el body, así que no
existe forma de modificar la preferencia de otro socio.
...
Errores: 400 `VALIDATION_ERROR` (falta `autoRenew` o no es booleano); 422
...
`POST /payments` (§5, campo `autoRenew` del request).
